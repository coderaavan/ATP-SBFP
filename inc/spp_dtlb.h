...
// Region size
//#define REGION_SIZE 524288   	// 512*1024
//#define LOG2_REGION_SIZE 19 	// 512KB region size

// Signature table parameters
#define ST_SET 1
#define ST_WAY 256
//#define ST_TAG_BIT (64-LOG2_REGION_SIZE)
#define ST_TAG_BIT (64 - 27)
...
//#define SIG_DELTA_BIT LOG2_REGION_SIZE - LOG2_PAGE_SIZE + 1
#define SIG_DELTA_BIT 16
...
public:
  ...
  uint64_t last_offset[ST_SET][ST_WAY], sig[ST_SET][ST_WAY],
      lru[ST_SET][ST_WAY];
  ...
  void read_and_update_sig(uint64_t page, uint64_t page_offset,
                           uint64_t &last_sig, uint64_t &curr_sig,
                           int64_t &delta);
};
...
public:
  ...
  void update_pattern(uint64_t last_sig, int curr_delta),
      read_pattern(uint64_t curr_sig, int64_t *prefetch_delta,
...
public:
  ...
  uint64_t sig[MAX_GHR_ENTRY], offset[MAX_GHR_ENTRY];
  ...
  void update_entry(uint32_t pf_sig, uint32_t pf_confidence, uint32_t pf_offset,
                    int pf_delta);
  uint32_t check_entry(uint64_t region_offset);
};
