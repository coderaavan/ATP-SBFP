...
public:
  ...
  uint64_t next_level_base_addr[NUM_ENTRIES_PER_PAGE]; // This address will not
                                                       // have page offset bits
...
public:
  ...
  uint64_t next_translation_virtual_address =
      0xf000000f00000000; // Stores the virtual address from which translation
                          // pages will reside
  ...
  void return_data(PACKET *packet), operate(),
      ...
      fill_mmu_cache(CACHE &cache, uint64_t next_level_base_addr,
  ...
  uint64_t get_index(uint64_t address, uint8_t cache_type),
      ...
      handle_page_fault(PAGE_TABLE_PAGE *page, PACKET *packet,
                        uint8_t pt_level);
  ...
  uint64_t va_to_pa_ptw(uint8_t cpu, uint64_t instr_id, bool translation_page,
                        uint64_t va, uint64_t unique_vpage, bool *page_swap);

  void write_translation_page(uint64_t next_level_base_addr, PACKET *packet,
                              uint8_t pt_level);
};
