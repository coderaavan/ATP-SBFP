...
//@Prashant
#define STLB_PB_MAX_SIZE 64

extern uint32_t SCHEDULING_LATENCY, EXEC_LATENCY, DECODE_LATENCY;
extern uint8_t TRACE_ENDS_STOP;
extern uint64_t cold_miss;
extern uint64_t sbfp_access;
extern uint64_t sbfp_pq_hit;
...
public:
  ...
  //@Prashant
  SBFP sbfp; 
  map<uint64_t, uint64_t> STLB_PB; //STLB Prefetch Buffer
  queue<uint64_t> aux_stlb_pb; // Helper to implement FIFO for PB

#ifdef ROB_STALL_STATS
  unordered_set<uint64_t> total_stall, stlb_miss_stall,
	  l1d_tr_miss_stall, l1d_load_miss_stall,
	  l2c_tr_miss_stall, l2c_load_miss_stall,
	  llc_tr_miss_stall, llc_load_miss_stall;
#endif
