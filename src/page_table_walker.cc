      ...
      0) // Handle pending request, only one request is serviced at a time.
  {
                                   ...
                                   // complete
    {
          ...
          !dropped_prefetch_request) // If translation complete
      {
        ...
        //ATP+SBFP
        #ifdef SBFP_ENABLE
        // Updating SBFP PQ with free PTEs at end of both demand and prefetch page walk
        ooo_cpu[cpu].sbfp.extract_and_fill(MSHR.entry[index].full_virtual_address >> LOG2_PAGE_SIZE, cpu); 
        #endif
...

    {
      ...
      if (address_pscl2 != UINT64_MAX) {
        ...
        if (CR3_addr == UINT64_MAX) {
          ...
          //ATP+SBFP
          #ifdef SBFP_ENABLE
          //Updating SBFP PQ with free PTEs at end of demand page walk
          ooo_cpu[cpu].sbfp.extract_and_fill(RQ.entry[index].full_virtual_address >> LOG2_PAGE_SIZE, cpu); 
          #endif
          //End of ATP+SBFP related changes
      ...
      if (address_pscl2 != UINT64_MAX) {
        ...
        if (CR3_addr == UINT64_MAX) {
          ...
          PQ.entry[index].data = next_level_base_addr << LOG2_PAGE_SIZE |
                                 (PQ.entry[index].full_virtual_address &&
                                  ((1 << LOG2_PAGE_SIZE) - 1));

          //ATP+SBFP
          #ifdef SBFP_ENABLE
          //Updating SBFP PQ with free PTEs at end of prefetch page walk
          ooo_cpu[cpu].sbfp.extract_and_fill(PQ.entry[index].full_virtual_address >> LOG2_PAGE_SIZE, cpu); 
          #endif
          //End of ATP+SBFP related changes
